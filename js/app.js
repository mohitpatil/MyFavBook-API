...
myBookApp.controller('myBookCtrl', function($scope, $http) {
    ...
    $scope.getBooks = function(search) {
        ...
        $http({
            method : "GET",
            url : "https://www.booknomads.com/api/v0/isbn/"+search //9789000035526
            //url : "books.json"
        }).then(function mySuccess(response) {

            myWelcome = (response.data);
            console.log(myWelcome.length);
            console.log('myWelcome Response', myWelcome, 'length : ', myWelcome.length);

            if(myWelcome.ISBN === search) {
                $scope.searchResult.push(myWelcome);
                } else {
